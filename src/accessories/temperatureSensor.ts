...
import { readAnalogInput, readAnalogValue, readAnalogOutput } from '../bacnet/bacnet';
...
export class BachomeTemperatureSensorAccessory {
  ...
  constructor(
    ...
    this.platform.log.debug(String(this.stateObjects.CurrentTemperature['type']));
    this.platform.log.debug(String(this.stateObjects.CurrentTemperature['instance']));
  ...
  async getCurrentTemperature(callback: CharacteristicGetCallback) {
    ...
    try {
      switch (this.stateObjects.CurrentTemperature['typeText']) {
        case 'AI': {
          let value = await readAnalogInput(this.ipAddress, this.stateObjects.CurrentTemperature['instance'], 85);
          // @ts-ignore
        ...
        case 'AO': {
          let value = await readAnalogOutput(this.ipAddress, this.stateObjects.CurrentTemperature['instance'], 85);
          // @ts-ignore
          value = value['values'][0]['value'];
        ...
        case 'AV': {
          const readProperty = await readAnalogValue(this.ipAddress, this.stateObjects.CurrentTemperature['instance'], 85);
          // @ts-ignore
